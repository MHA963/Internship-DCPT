...
trial_folder = Path('E:/Jasper/DBCGRT/Vejle/BCCT HYPO Vejle 03.2018') # folder to run through
new_save_folder = Path('E:/Jasper/DBCGRT/MHA/processed_images/Vejle_processed') # folder to save images to 
...
def update_counters(predicted_label, image_name):
    ...
    if predicted_label == 'armsUp' and image_name.endswith('_2'):
        armsUp_counter += 1
        #print(f"positive armsUp_counter incremented {armsUp_counter}")
    elif predicted_label == 'armsDown' and image_name.endswith('_1'):
        armsDown_counter += 1
        #print(f"positive armsDown_counter incremented {armsDown_counter}")
    elif predicted_label == 'Side' and image_name.endswith('_3'):  # Update this number if necessary
        sideView_counter += 1
        #print(f"positive sideView_counter incremented {sideView_counter}")
    elif predicted_label == 'armsUp' and not image_name.endswith('_2'):
        false_armsUp_counter += 1
        #print(f"negative armsUp_counter incremented {false_armsUp_counter}")
    elif predicted_label == 'armsDown' and not image_name.endswith('_1'):
        false_armsDown_counter += 1
        #print(f"negative arms_down_counter incremented {false_armsDown_counter}")
    elif predicted_label == 'Side' and not image_name.endswith('_3'):  # Update this number if necessary
